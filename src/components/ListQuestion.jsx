    ...
    <>
      <div className="w-full space-y-5 lp:w-[631px] dk:w-[867px]">
        {dataQuestions.map(({ title, desc, subTitle }, index) => (
          <div
            key={index}
            className="rounded-[10px] border-[1px] border-white-95 space-y-5 p-6 lg:mt-0 lp:p-7.5 "
          >
            <div className="flex justify-between space-x-7.5 items-center">
              <p className="font-medium  text-[16px] leading-6 lp:text-lg lp:leading-[27px] dk:text-xl dk:leading-7.5">
                {title}
              </p>
              <button
            ...
            {activeQuestion === index && (
              <div className="border-t-[1px] border-white-95">
                <p className="text-grey-30 my-5 lp:my-7.5 dk:my-12.5 text-body">
                  {desc}
                </p>
                <div className="bg-white-95 py-3 px-5 rounded-md lp:py-[16px] lp:px-6 dk:py-5 dk:px-7.5">
                  <div className="flex justify-between items-center">
                    <p className="font-medium text-body">{subTitle}</p>
